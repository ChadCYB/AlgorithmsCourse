...
public class Map {
	...
	public static double[][] dist(ArrayList<Place> p){
		int point_mount = p.size();
		double[][] distArr = new double[point_mount][point_mount];
		for(int i=0 ; i<point_mount ; i++){
			for(int j=i ; j<point_mount ; j++){
				if(i == j){			
					distArr[i][j] = 0.1000000000000001d;
				}else{
					distArr[i][j] = Map.dist(p.get(i), p.get(j));
